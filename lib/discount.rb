...
module Discount
  ...
  class << self
    ...
    def calculate_percentage_discount(discountable_amount, role)
      case role
      when Role::STAFF
        calculate_staff_discount(discountable_amount)
      when Role::AFFILIATE
        calculate_affiliate_discount(discountable_amount)
      when Role::LOYAL 
        calculate_loyality_discount(discountable_amount)
      else
    ...
    def calculate_staff_discount(discountable_amount)
      (discountable_amount * Discount::STAFF_DISCOUNT / 100).to_f.round(2)
    end
    
    # calculates the affiliate discount for retailer affiliates 
    def calculate_affiliate_discount(discountable_amount)
      (discountable_amount * Discount::AFFILIATE_DISCOUNT / 100).to_f.round(2)
    end
    
    # calculates the loyality discount for the loyal user
    def calculate_loyality_discount(discountable_amount)
      (discountable_amount * Discount::LOYALITY_DISCOUNT / 100).to_f.round(2)
    end
